...
L = 1000; lags = 0:L
A = zeros(L+1,2*d)
A[:,1] = autocov(sig[2,:],lags)
A[:,2] = autocov(sig[3,:],lags)
A[:,3] = autocov(sig_rm[1,:],lags)
A[:,4] = autocov(sig_rm[2,:],lags)


plot(lags, A,
    line = (2,[:solid :solid :dot :dot]),
