...
function z_crossspect_fft(
    ...
    for i = 1 : d
        for j = 1 : nu
            C_smoothed[i,j,:] = Lam .* ( (j <= d) & (i != j) ?
                    zeros(size(lags)) :
                    my_crosscov(sig[i,1:steps],pred[j,1:steps],lags) )
        end
    end
...
function vector_wiener_filter_fft(pred::Array{Complex{Float64},2}, sig,
    ...
    for i = 1 : nu
        for j = 1 : nu
            R_pred_smoothed[i,j,:] = Lam .* ( (i <= d) & (j <= d) & (i != j) ?
                    zeros(size(lags)) :
                    my_crosscov(pred[i,1:steps],pred[j,1:steps],lags) )
        end
    end
