...
function my_KSE_solver(
    ...
    LR = h*L*ones(M)' + ones(2N+1)*r' # the second dim varies r the first vaeries L
    ...
    a  = Complex.(zeros(2N+1))
    b  = Complex.(zeros(2N+1))
    c  = Complex.(zeros(2N+1))
    Nv = Complex.(zeros(2N+1))
    Na = Complex.(zeros(2N+1))
    Nb = Complex.(zeros(2N+1))
    Nc = Complex.(zeros(2N+1))
    ...
    vv = complex(zeros(2N+1, n_obs+1)); vv[:,1]= v
    uu = zeros(2N+1, n_obs+1); uu[:,1]= u
